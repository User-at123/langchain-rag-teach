...
│
├── V10.2 Web 问答提速（性能优化，已实施）
│   ├── 痛点：枚举/集合类问题（如"李云龙妻子都是谁"）15~20s——
│   │   3 次串行 LLM + reranker 100 对精排 + 5 路子查询串行召回
│   ├── 四项优化：回答缓存（相同问题秒回）/ 并行粗召回（线程池）/
│   │   候选 20→15 条每路 / 前端等待提示文案
│   └── 验证：pytest 24 用例全绿（10.18s）；不相关问题的"快"源于子查询降级单路
│
├── V11.0 多轮记忆 + 三路路由（第 9 步，已实施）
│   ├── 会话记忆：session_id + 内存 sessions（历史 6 轮 + 上轮 SQL 结果跨轮状态）
│   ├── 路由器升级三路：vector / sql / memory（history_hint 只参与判断不进回答）
│   ├── memory 路：补全器改写指代 → 重路由；引用上轮结果直接答（不检索不查库）
│   ├── 接入：CLI 多轮循环 + Web session_id（localStorage 持久化）
│   └── 验证：pytest 28 用例全绿（7.36s，新增 4 个记忆链路测试）
│
├── V12.0 Agent 工具化（第 10 步，已实施）
│   ├── 路由器升级四路：vector / sql / memory / agent（从"唯一决策者"降级为"调度员"）
│   ├── 手写 ReAct 循环（零新依赖，不引 langgraph）：query_sql / retrieve_vector /
│   │   read_memory 三工具，LLM 多轮自主编排（先查再分析再生成）
│   ├── 兜底：Agent 超步（MAX_AGENT_STEPS=4）/ 输出不可解析 → 回退快路径 _run_route
│   └── 验证：pytest 34 用例全绿（7.26s，新增 6 个 Agent 链路测试）
│
├── V11.1 召回修复：抽象问法具象化 + 粗召回扩容（已实施）
│   ├── 场景：问"田雨最后的结局"答"查不到"——原文 12906 行明确写"田雨在狱中割腕自杀"
│   ├── 根因链：抽象词（结局）vs 原文具体写法（割腕自杀）→ 子查询全抽象 → BM25 零匹配；
│   │          罕见词 chunk 在 RRF 融合排 25 → recall_top_n=15 直接截断丢答案
│   ├── 机制版改写：prompt 加"抽象问法必须具象化"规则——联想具体词并分散到不同子查询、
│   │          至少一路保留原关键词、专有名词原样（不写死"结局→自杀"映射，通用而非补丁）
│   ├── V11.1.1 补坑：改写随机性——LLM 联想词表不稳定（"结局"有时漏"自杀"），
│   │          prompt 改为按"方向"约束：死亡类抽象词必须覆盖正常+非正常死亡两方向
│   ├── V11.1.2 补坑：方向拆解通用化——用户质疑"只约束死亡类，其他抽象词怎么办"；
│   │          第 4 条升级为所有抽象词先拆 2-3 个语义方向各成一路，死亡类降级为重要案例
│   ├── V11.1.3 补坑：top_n 6→8——「田雨死了吗」割腕 chunk 精排第 2 但只在一路出现，
│   │          RRF 融合分 1/62 排第 7，top6 被"多路同现"常规 chunk 占满截断丢答案
│   ├── 调参：recall_top_n 15→30、per_query_top_n 3→5（实测「田雨 结局 自杀」RRF 排 25）
│   └── 验证：42 用例全绿（test_multi_query_rewrite.py 8 个回归测试 + top_n==8 断言固化）
│
├── V12.1 抽象问法具象化改造：枚举式规则 → 唤醒式预演（已实施）
│   ├── 场景：问抽象类问题（结局/心情/爱情/去了哪里等）——prompt 只对枚举过的抽象词
│   │          可靠（结局/婚姻/事业/战斗结果），"心情/爱情/去向"等未枚举类型未必触发拆解
│   ├── 认知：穷尽不了抽象类型；DeepSeek 网页版靠模型原生推理而非枚举规则——
│   │          检索器（BM25/向量）不推理，抽象词必须借 LLM 转成字面词才能命中，
│   │          但"唤醒联想"比"枚举分类"更接近模型原生能力
│   ├── 改法：第 4 条从"判断是否抽象词（分类）"改为"像读者一样预想原文写法（生成）"——
│   │          对结局/心情/爱情/去向等任意抽象说法，预想具体词并分散到不同子查询；
│   │          示例只是示范"不局限于本示例、随问题自由预想"（防词表化）
│   ├── 保留：第 5 条死亡类强锚（正常/非正常死亡双向）不放松——实测最易漏的高价值案例
│   └── 验证：44 用例全绿（test_multi_query_rewrite.py 新增 2 例：唤醒式措辞 + 死亡类强锚回归）
│
...
### V10.2 Web 问答提速：缓存 + 并行召回 + 候选减量（性能优化，已实施）

**改动内容**
- 回答缓存：`ask()` 加内存缓存 `_answer_cache`（上限 64 条，dict 保序超限弹最旧）——
  相同问题直接命中秒回，省去整套检索 + 3 次 LLM 调用；**注入模式（测试）不缓存**，
  保证 mock 链路每次走真实逻辑（这是缓存能安全加上的前提）
- 并行粗召回：`MultiQueryRetriever._get_relevant_documents` 里 5 路子查询从**串行 invoke**
  改为线程池并行（`ThreadPoolExecutor`）；关键细节：**第一个子查询在主线程先跑**，
  顺带触发 BM25 懒索引构建（jieba 分词 2541 片段），避免多线程同时首调重复建索引 + GIL 争抢
- 候选减量：`recall_top_n` 20→15（构造时传参），5 路候选 100→75 对（去重后约 40 对），
  reranker 本地 CPU 精排省 ~2s；质量靠"拆子查询已带成员专有名词"兜底（枚举问题每路前 15 条足够）
- 前端等待提示：`index.html` 占位气泡从"…"改为"正在检索知识库并生成回答（约 10~20 秒）…"，
  让用户对等待有预期（系统在工作，不是卡死）

**为什么慢 / 为什么不相关问题快（性能分解）**
- "李云龙的妻子都是谁"（枚举类）：路由器 LLM 1~3s + 拆 5 路子查询 LLM 1~3s +
  5 路串行召回 2~5s + reranker ~53 对 5~6s + 主 LLM 生成 2~5s ≈ 15~20s
- 不相关问题（如"特朗普政绩"）：拆子查询失败 → 降级成原问题单路（rag.py 解析失败
  `return [query]`）→ reranker 只算 20 对 ≈ 2s；主回答"资料里没有"很短 → 总耗 5~8s
- 结论：大头永远是 reranker（本地 CPU 交叉编码器）+ 3 次串行 LLM（网络往返），
  并行只能压缩召回段，减候选量压缩 reranker 段

**使用的工具**
- `concurrent.futures.ThreadPoolExecutor`：并行粗召回（标准库，零新增依赖）
- `dict` 保序 + `next(iter(...))` 弹出最旧：手写简单 FIFO 缓存，不引第三方

**注意事项**
- 并行召回的前提：BM25 索引已构建（首个子查询预热）；若未来把 BM25 改成启动即建索引，
  可去掉预热直接全并行
- 缓存只在进程内有效（重启即失）；注入模式不缓存，测试不受缓存状态干扰
- 更进一步的感知提速（未实施）：流式输出（SSE，打字机效果）——主 LLM 生成段
  从"等 3~5s 全出"变"边生成边显示"，需改 app.py + index.html，改动较大留作候选

**验证结果**
- pytest 24 用例全绿（10.18s）；无新增 lint 错误
...
**改动内容**
- 会话记忆：`ask()` 新增 `session_id` 参数，非 None 时启用记忆——模块级 `sessions`
  dict 按 session_id 存 `{"history": [(role, text), ...], "last_result": ...}`，
  历史截断保留最近 6 轮（`MAX_HISTORY_ROUNDS`）；**无 session_id 时行为与 V10 完全一致**
- 跨轮状态：`last_result` 暂存最近一次 SQL 查询的格式化结果（以 `[数据库查询]` 开头判定）；
  追问"刚才结果里有多少家"时直接取用，**不再重复查库**
- 三路路由：`router_prompt` 从二选一升级为 vector / sql / memory 三选一；
  关键设计——**历史只以 `history_hint` 喂给路由器判断"是否引用上轮"，不直接参与
  vector/sql 链的回答**（避免污染检索语义，呼应 V7.8 认知边界）
- memory 路（`_answer_with_memory`）：记忆补全器（`memory_prompt`）把带指代的问题
  改写成自包含问题并输出 JSON `{rewritten, base}`，然后按 base 分流：
  - `base=vector` → 改写后走文档检索链
  - `base=sql` → 改写后走 SQL 链（失败仍降级向量链）
  - `base=last_result` → 基于上轮结果直接回答（`qa_from_result_prompt`，不检索不查库）
  - JSON 解析失败 → 降级把原问题走完整路由，不崩溃
- Web 接入：`app.py` 的 `/ask` 支持 `session_id`（未传由后端 `uuid4` 生成并回传）；
  `index.html` 用 `localStorage` 持久化会话 ID——刷新页面多轮记忆仍延续
- CLI 改造：从单次提问改为多轮循环（固定 `session_id="cli"`），输入 `exit/quit/退出` 结束，
  方便直接验证"先问再追问"的完整链路

**为什么 memory 是第三条路（承接 V7.8 / 核心概念 5）**
- "那她们结局如何"这种指代，脱离上轮历史无法独立理解 → 必须结合会话历史改写后重路由；
- "刚才结果里有多少家客户"——答案在**上轮 SQL 导出的结果里**，知识库里没有这个中间结果，
  走向量/SQL 必然命中不了 → 只能读跨轮状态 `last_result`；
- 路由器"看历史"和回答链"用历史"是两回事：历史进判断（history_hint）是为了识别
  memory，而 vector/sql 链的回答仍只用当前问题（避免历史文本污染检索语义）

**使用的工具**
- `ChatPromptTemplate` + 手写 JSON 解析（`_parse_memory_plan`，容忍模型输出夹带说明文字）：
  记忆补全器，零新增依赖
- 内存 dict 会话存储：`sessions.setdefault(session_id, {...})`，与回答缓存同生命周期
- 前端 `localStorage`：浏览器端会话 ID 持久化（无 Cookie/无登录的最小方案）

**注意事项**
- 踩坑：`memory_prompt` 的 system 文案里 JSON 示例 `{"rewritten": ...}` 的花括号
  会被 LangChain 当成模板变量 → 必须转义为 `{{...}}`（或改用文字描述格式）
- 会话存储是进程内内存：重启服务即清空（与回答缓存、CLI 会话同生命周期）；多进程部署
  需换共享存储（Redis/数据库），教学本地单进程足够
- 缓存与记忆的顺序：命中回答缓存时也会写入会话历史（`_record_session`），
  否则追问指代会因缺了上一轮记录而接不上
- 测试安全：注入模式 + 无 session_id 时完全走 V10 旧逻辑，现有 24 用例零改动通过；
  新增 4 个记忆用例用 4 桶 FakeLLM（router/memory/sql/answer），不调真实 API

**验证结果**
- pytest 28 用例全绿（7.36s）：新增 test_memory.py 覆盖 指代改写走向量链 /
  跨轮状态 last_result 直答 / 补全器解析失败降级 / 无 session 不写会话
- 无新增 lint 错误
...
**改动内容**
- 路由器升级四路：vector / sql / memory / agent——**不删除原三路**，而是把路由器
  从"唯一决策者"降级为"调度员"：单跳问题走快路径（原固定链路），多跳/组合问题
  （含"先…再/然后/接着/结合…与…"信号）才进 Agent，Agent 失败回退快路径
- 手写 ReAct 循环（`_run_agent`，零新增依赖，不引 langgraph）：每轮把完整对话
  （SystemMessage + 历史消息 + 新 Observation）直接传给 LLM，循环由"看到
  Observation 再决策"驱动；`MAX_AGENT_STEPS = 4` 防死循环
- 三个工具（`_call_agent_tool` 调度）：
  - `query_sql(问题)` → 复用 Text-to-SQL 链（sql_answer），成功同步更新会话
    `last_result`（跨轮状态），Agent 后续 read_memory 可读到本轮结果
  - `retrieve_vector(问题)` → 复用文档检索链（chain）
  - `read_memory()` → 返回会话历史 + 上轮结果（`_format_session_context`）
- 输出解析：`_extract_final`（Final Answer）/ `_parse_action`（Action + Action Input）
  ——手写正则，容忍"Action: xxx / Action Input: yyy"中文冒号变体
- 兜底双保险：① 输出既无 Action 也无 Final Answer → 立即回退；② 超步数未收敛
  → 回退。回退目标都是 `_run_route`（快路径再判一次），保证 Agent 异常不拖垮问答

**为什么保留路由器而不是删除（关键设计决策）**
- "路由交给 LLM"在第 9 步已是事实（router_prompt | llm），第 10 步真正的区别是
  **多轮循环 vs 一次性判断**，不是"LLM vs 非 LLM"；
- Agent 每步至少 2 次 LLM 调用（Thought + Action），单跳问题走 Agent 更慢更贵
  → 快路径保留是性能需要；
- Agent 可能不调工具直接瞎编 / 陷入循环 / 输出不可解析 → 固定链路 + 降级逻辑
  是确定性兜底网，删了它 Agent 一崩全崩；
- 测试与教学：28 个存量用例大量依赖路由分桶；保留两套才能对比"确定性路由 vs
  自主编排"两种范式，这是本项目最有价值的认知点

**使用的工具**
- `langchain_core.messages`（SystemMessage / HumanMessage / AIMessage）：ReAct 循环
  的消息维护，零新增依赖
- 手写正则解析（`re`）：Final Answer / Action 提取，与项目"手写 JSON 解析"风格一致
- 调用方式细节：循环里用 `_llm(messages)`（`__call__` 等价 `invoke`）而非
  `.invoke()`——真实 ChatOpenAI 与测试裸 callable 都兼容，注入模式通用

**注意事项**
- 踩坑：FakeLLM 只有 `__call__` 没有 `invoke` → 循环里必须用 `_llm(messages)`，
  `.invoke(messages)` 会让注入模式的测试 AttributeError
- `query_sql` 工具在 `_db is None`（注入模式未传 db）时返回提示而非崩溃——
  工具调用异常不应拖垮整个问答（兜底哲学）
- Agent 循环内 query_sql 同步写 `last_result` 是特意为之：保证 Agent 内部
  query_sql → read_memory 两步协作能读到同轮结果
- 路由器优先级：agent 判断（多步/组合信号）排在 sql/memory 之前，避免
  "先统计各行业…再…" 这类问题被 sql 规则抢走

**验证结果**
- pytest 34 用例全绿（7.26s）：新增 test_agent.py 覆盖 多跳编排（query_sql 真实执行
  + Final Answer）/ retrieve_vector 工具 / read_memory 工具（结合历史）/
  超步回退快路径 / 输出不可解析回退 / 无 session 时 agent 兼容
- 无新增 lint 错误
...
**改动内容**
- 修复场景：问"田雨最后的结局"答"查不到"，但原文（OCR 缓存第 12906 行）明确写了
  "田雨在狱中割腕自杀"——知识库有答案，是检索没召回
- 改动两处：
  1. **机制版改写 prompt**（`MultiQueryRetriever._generate_sub_queries`）：
     新增"抽象问法必须具象化"规则——问题里的抽象词（结局/下场/命运/怎么样了）要联想
     原文更可能出现的具体词（去世/自杀/死亡/被捕…），且多个候选具体词分散到不同
     子查询；至少一路保留原问题原始关键词；专有名词原样带上
  2. **召回参数**：`recall_top_n` 15→30、`per_query_top_n` 3→5

**踩过的坑（重点，三层根因叠加）**
1. **语义鸿沟（主因）**：用户问"结局"，原文写"割腕自杀"——BM25 对"结局"零匹配
   （全书 8 次"结局"没一次和田雨有关）；bge-small-zh 向量也被高频"田雨"主导
2. **RRF 融合挤压**：实测"田雨 自杀"单路 RRF 全量排第 23、"田雨 结局 自杀"排第 25——
   罕见词 chunk 只有 BM25 一路贡献分（1/61），被"BM25+向量双命中"的常规 chunk 挤出
3. **截断丢答案**：`recall_top_n=15` 把第 25 名直接截掉——reranker 再强也救不回
   没召回的东西（`per_query_top_n=3`、`top_n=6` 都是后续雪上加霜，根源在粗召回）
- **只改 prompt 不够**：第一版机制版改写让 LLM 联想"去世/死亡"，但没联想"自杀"
  （LLM 认为"去世"更通用），5 路子查询全没带"自杀"→ 必须同时调大粗召回漏斗
- **改写随机性（V11.1.1 补坑）**：机制版规则正确，但"多候选词覆盖"仍依赖 LLM
  单次采样的联想运气——实测「田雨和秀芹的结局分别怎么样」那次 5 路子查询全没带
  "自杀"（去世/死亡/牺牲/被捕/怎么样），割腕 chunk 一路未召回；而「田雨的结局」
  那次恰好联想出"自杀"，就答对了。同一个抽象词，两次联想词表不同
  → 修：prompt 把"方向"规则写死（不是词表）——死亡/生命状态类抽象词必须覆盖
  「正常死亡（去世/病故）+ 非正常死亡（自杀/牺牲/被杀/处决/自尽）」两个方向，
  各成一路。实测 3 连跑全部覆盖"自杀"，割腕 chunk 全部进 top6 ✅
- **方向规则只覆盖死亡类（V11.1.2 补坑，用户质疑触发）**：用户问"强化是不是只针对
  死亡类型？其他抽象词遇到同类 LLM 随机性怎么办？"——确实，第 5 条只约束
  「结局/命运」这类词，而"婚姻/事业/战斗结果"等抽象词若出现同样的漏词随机性
  依然无解 → 修：把「方向拆解」从死亡类专属升级为**通用规则**——
  第 4 条改为：任何抽象/模糊词（结局/下场/命运/怎么样了/发生了什么/后来如何，
  或婚姻/事业/财产/战斗结果等）都必须先拆成 2-3 个语义方向（如"婚姻"→ 结婚/
  离婚/分居；"事业"→ 升迁/贬职/免职；"战斗结果"→ 胜利/战败/惨胜；"结局"→
  正常死亡/非正常死亡），每个方向用不同的具体词各写一路子查询；
  第 5 条死亡类降级为「方向拆解的重要案例」（保留强约束防回归）
- 教训：**"改写质量"和"召回漏斗宽度"要配套调**，只调一头都救不回罕见词 chunk；
  也印证"召回（Recall）优先于精排（Precision）"——召回漏了，精排无从谈起；
  再进一步：**"让 LLM 联想具体词"要按"方向"约束而不是按"词表"约束**——
  词表是补丁，方向是机制（每个"结局"都可能对应正常/非正常死亡两种写法）；
  更彻底：**方向约束也要"通用化"而非"逐类写死"**——只给死亡类写规则仍是
  另一种词表（按类型枚举），要对"所有抽象词都要求做方向拆解"才是真正机制；
  代价是 prompt 变长稀释注意力，靠"方向拆解 + 示例 + 不局限于本示例"三件套平衡

**使用到的工具**
- `CrossEncoder`（bge-reranker）逐环节打分：粗召回位置 → 精排位置 → 截断点
- 探针脚本追踪全链路：子查询生成 → 单路粗召回 → RRF 全量排名 → 精排截断

**验证结果**
- 实测：机制版改写后子查询 = "田雨 结局 去世 / 田雨 李云龙 妻子 结局 / 田雨 最后
  死亡 原因 / 田雨 晚年 生活 去世 / 田雨 结局 是什么"（5 路分散覆盖具体词）；
  "割腕自杀"chunk 进最终 top6（第 2 条），LLM 能答出"田雨在狱中割腕自杀"
- pytest 42 用例全绿：新增 `tests/test_multi_query_rewrite.py` 8 个回归测试
  （prompt 规则固化 + 死亡方向规则固化 + 通用方向拆解规则固化 + 召回参数固化 +
  解析降级），防止修复被无意改回去

---

### V11.1.3 最终视野扩容：top_n 6→8（已实施）

**修复场景**
- 用户连续问「她们的结局都是什么 / 田雨的结局 / 田雨死了吗」三个问题，前两个答对、
  「田雨死了吗」答"没提到"——但知识库明确写了"田雨在狱中割腕自杀"。
- 用户质疑"又改回去了，不如上一轮改动之前的代码"（git 确认代码未回退，
  是真实新失败）。

**根因（实测钉死，与 prompt 无关）**
- 子查询方向拆解完全正常：5 路 = 死亡/自杀/被杀/关系/结局+原始词"死了吗"
- 割腕 chunk 在「田雨 自杀 牺牲 被杀 处决」路**精排第 2**（bge-reranker 分 0.9375）
- **但它只在这一路出现**（其他路不带自杀词、向量召回也不同）→ RRF 融合分
  1/(60+2) ≈ 0.0161，全局排第 7
- top6 被"多路同现"的常规 chunk 占满（关系路 5 个 0.99 分 chunk 各 1/60~1/64）——
  **top_n=6 把第 7 名截断**，reranker 分再高也救不回
- 对比「田雨的结局」能答对：那次割腕 chunk 融合排名更靠前（进前 6）

**修复**
- `top_n` 6→8：最终视野多 2 条，对"只在一路出现但高度相关"的 chunk 更宽容。
  LLM 上下文仅多 ~1k 字（deepseek 可承受）。这是通用机制（任何"单路命中但关键"
  的信息都受益），不是针对割腕的补丁。
- 测试固化：`test_default_recall_params_not_regressed` 加 `top_n == 8` 断言；
  同时给测试 `_make()` 显式传 `top_n`（此前靠类默认值 6，与真实链路 `_build_retrievers`
  显式传参不一致——测试首跑就暴露了这一点）

**验证**
- 同一 session 连问三题（复现用户场景）：三题全答对——
  秀芹被俘牺牲 + 田雨狱中割腕自杀 + 冯楠赵刚去世；「田雨死了吗」答"在狱中割腕自杀"✅
- 独立 session 单问「田雨死了吗」：答"确实死了，割腕自杀"✅（路由 vector，不依赖记忆）
- pytest 42 全绿（8 个回归测试，新增 top_n==8 断言）

**教训**
- **漏斗参数要检查"最细的那一级"**：V11.1 调了 recall_top_n（粗召回）、
  V11.1.1 调了 prompt（改写）、这次才发现 top_n（最终视野）也是截断点——
  三个漏斗都窄，任何一个都能丢答案；「田雨死了吗」恰恰暴露了 top_n 这一级
- **"只在一路命中" vs "多路同现"**：RRF 融合天然偏爱多路同现的 chunk，
  单路高相关 chunk 融合分被稀释——top_n 放宽是最低成本的兜底
- 用户侧三个问题全错 vs 探针三题全对 → 需排查 app 是否重启（常驻进程不加载
  新代码）、会话记忆污染、LLM 单次采样随机性；机制只能降概率，知识库层摘要
  （docs/sample.md 加"田雨…狱中割腕自杀"条目）才是 100% 兜底（待拍板）

---

### V12.1 抽象问法具象化改造：枚举式规则 → 唤醒式预演（已实施，2026-09-02）

**改动内容**
- 背景：V11.1 的"方向拆解"靠 prompt **枚举抽象词类型**（结局/婚姻/事业/战斗结果）去引导
  LLM——结果只对枚举过的类型可靠。用户点破：问"心情/爱情/去了哪里"等**未枚举**的抽象
  说法，LLM 未必触发拆解，照样漏词；且"给每类抽象词补规则"根本穷尽不了
- 关键认知（回应"DeepSeek 网页版为什么不用枚举也能答好"）：
  - DeepSeek 网页版是**纯推理**——所有知识在模型脑子里，抽象词由模型原生理解，无需枚举；
  - 本项目是 **RAG**——答案必须**检索召回**才看得到，而检索器（BM25/向量）**不推理**，
    "结局"与"割腕自杀"字面零重合（V11.1 实测），抽象词必须借 LLM 转成字面词才能命中；
  - 所以项目必须"逼 LLM 拆解抽象词"，但正确的做法是**唤醒模型的原生联想**，
    而非**枚举分类**（词表永远不完整）
- 改造（`rag.py` `_generate_sub_queries` 第 4 条，核心一行措辞的变化）：
  - **改前（分类）**："如果问题用了抽象词（结局/婚姻/事业…），先拆成 2-3 个方向"
    ——依赖 LLM 判断"这是否需要拆的抽象词"，词表外识别不积极
  - **改后（生成）**："像一位熟悉作品的读者那样，预想原文更可能用哪些具体词/情节/情境
    来表达这个抽象概念……凡抽象概括处都应如此预想"
    ——把"是否该拆"（分类，易漏）换成"一定能预想具体写法"（生成，模型天生擅长），
    从而覆盖**任意**抽象类型（结局/心情/爱情/去向…），不再依赖词表
  - 示例升级：结局/命运、心情/情绪、爱情/关系、去向/下落、婚姻/事业/战斗结果——
    用 `→ 具体词` 带出**类别**而非**穷举**，且声明"不局限于本示例、随问题自由预想"防词表化
  - **保留第 5 条死亡类强锚**（正常+非正常死亡双向必须覆盖）不放松——这是实测最易漏的
    高价值案例（「田雨死了吗」漏割腕 chunk 的历史教训），唤醒式不能替代这条保险

**为什么这样是"借鉴官方"而非"回到词表"**
- 与 DeepSeek 网页版的差距本质是"多了个不会推理的检索环节"，不是模型能力差
- 因此目标不是让模型"知道更多抽象词规则"，而是**把检索环节前的那次拆解，交给模型的
  原生推理**——用任务描述（预想原文写法）唤醒，而不是喂词表
- 这是 V11.1.2"方向拆解通用化"的进一步：从"对死亡类等枚举都要求拆方向"推进到
  "对所有抽象说法都默认预演原文写法"，词表不再是入口

**验证结果**
- pytest 44 用例全绿（7.42s）：`test_multi_query_rewrite.py` 新增 2 例
  - `test_prompt_uses_readers_imagination_for_any_abstract_word`：断言 prompt 用
    "预想原文/读者"唤醒措辞，且示例覆盖心情/爱情/去向等非枚举类别、防词表化
  - `test_prompt_death_class_remains_strong_anchor`：断言死亡类强锚未因改唤醒式而放松
- 顺带修复 `app.py:31` 拼写 `@asynccontextmanage` → `@asynccontextmanager`
  （装饰器少了个 r，导致 import app 报 `NameError`、pytest 收集失败）

**注意事项 / 待办**
- 测试用的是回显 FakeLLM，只锁定了 prompt 文本；**真实 DeepSeek 是否真会拆出
  "心情→愤怒/悲痛/流泪"等方向，需手动探针验证**（见 README/improvements 里的探针命令）
- 纯"唤醒式"仍有 LLM 采样随机性（V11.1.1 同款风险），死亡类靠强锚兜底，其他类型靠
  "方向示例 + 防词表化声明"降概率；若要 100% 兜底仍需知识库层摘要（待拍板，见 V11.1.3）
...
| 9 | 多轮记忆 + 跨轮状态（引用上轮结果）（✅ 已实施 V11.0，2026-08-27） | 内存会话 dict / 记忆补全器（ChatPromptTemplate） |
| 10 | Agent 工具化（多步任务自主编排）（✅ 已实施 V12.0，2026-08-27） | 手写 ReAct 循环 + query_sql / retrieve_vector / read_memory 三工具 |
...
| （已完成，待提交） | 多轮记忆 + 三路路由：ask() 加 session_id + 内存会话（历史 6 轮 + last_result 跨轮状态），路由器升级 vector/sql/memory，记忆补全器改写指代 / 引用上轮结果直答，CLI 多轮循环 + Web localStorage 会话 | V11.0 |
| （已完成，待提交） | Agent 工具化：路由器升级四路（+agent），手写 ReAct 循环（MAX_AGENT_STEPS=4）+ query_sql / retrieve_vector / read_memory 三工具，Agent 超步/不可解析回退快路径 | V12.0 |
| （已完成，待提交） | 召回修复：抽象问法具象化（方向拆解）+ 粗召回扩容（recall_top_n 30 / per_query_top_n 5）+ 最终视野扩容（top_n 8） | V11.1 |
| （已完成，待提交） | 抽象问法具象化改造：枚举式规则 → 唤醒式预演（像读者一样预想原文写法，覆盖任意抽象类型如心情/爱情/去向；死亡类强锚保留） | V12.1 |
