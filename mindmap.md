...
└── V8+ 未来规划（见 improvements.txt）
    ...
    └── 12  增量索引（方案 B，待确认）：.index_state.json + 文件级差异，免全量重建
```
...
| 12 | 增量索引（方案 B，已记录待确认） | .index_state.json + Chroma 按 source 增删，免全量重建 |
...
**规划 B：增量索引（已记录，待确认后实施；对应 improvements 第一部分第 5 条）**
- 痛点：V5 起的设计缺陷——`chroma_db` 存在直接加载、不存在全量重建，中间没有
  "哪些文件是新的"判断；加新文件必须删库重建，扫描 PDF 每次全量重 OCR（最贵一环反复支付）
- 改动清单（V8.1，确认后实施）：
  1. **状态文件**：`.index_state.json`，存 `{文件路径: {hash, 时间}}`——记录每个文件的指纹
  2. **rag.py 启动流程**：`chroma_db` 存在 → 比对差异 → 增量处理（新增/变更/删除）；
     无状态文件（旧索引）→ 提示做一次"一次性全量校准"后转增量
  3. **Chroma 操作**：`add_documents` 追加、`delete(where={"source": ...})` 按来源删
  4. **BM25 数据源改造（关键联动）**：现在 BM25 每次由内存 chunks 重建，增量后内存里
     不再有全量 chunks，必须改为从 Chroma 全量取回文本重建——
     顺便讲清"向量索引持久化 / BM25 无状态"这对概念
  5. **文档同步**：mindmap（V8.1）、improvements、README
